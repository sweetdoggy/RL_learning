...
class A2COFF:
    ...
    def sample(self):
        state = [np.random.randint(0, self.env.grid_size[0]), np.random.randint(0, self.env.grid_size[1])]
        steps = 0
    ...
    def update(self, transition_dict):
        ...
        importance_weight=action_prob/(1/self.env.action_space_size)
        value_state=self.value_net(state)
        advantage = td_target - value_state
        policy_loss = torch.mean(torch.log(action_prob) * (-advantage.detach())*importance_weight.detach())
        value_loss = torch.mean(F.mse_loss(value_state, td_target.detach()))
