...
class expected_sarsa:
    def __init__(self, env, alpha=1e-2, gamma=0.9, epsilon=0.01):
        self.env = env
        self.policy = random_policy(env.grid_size[0], env.grid_size[1], env.action_space_size)
        #qsa初始化全0会收敛很慢，可以随机初始化，但是不要全0
    ...
    def policy_evaluation(self):
        ...
        new_qsa[x, y, action] = self.qsa[x, y, action] - self.alpha * (
                    self.qsa[x, y, action] - (reward + self.gamma * v))
    ...
    def policy_improvement(self):
        ...
        for i in range(self.env.grid_size[0]):
            for j in range(self.env.grid_size[1]):
                ...
                policy[i, j, :] = self.epsilon / self.env.action_space_size
                policy[i, j, best_action] += (1 - self.epsilon) / len(best_action)
        return policy
    ...
    def learning(self):
        ...
        while True:
            ...
            if np.max(np.abs(self.qsa-self.new_qsa))<1e-5:
                self.stable_steps+=1
            else:
                self.stable_steps=0
            ...
            action_probs=self.policy[self.state[0], self.state[1]]
            self.action=np.random.choice(len(action_probs), p=action_probs)
            self.epsilon *= 0.99
