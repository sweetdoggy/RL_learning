...
import time
def truncated_policy_iteration(env,gamma=0.9,truncated_rounds=20):
...
env=grid_env(grid_size=[20,20],target=[3,2],forbidden=[[1,1],[2,4],[3,3],[4,1]],start=[0,0])
start=time.time()
V,policy,total_rounds=truncated_policy_iteration(env)
end=time.time()
